...
@WebServlet(name = "BackBookServlet")
public class BackBookServlet extends HttpServlet {
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            userService.backBook(form);
            response.getWriter().print("恭喜，还书成功！");
            response.sendRedirect(request.getContextPath() + "/Chapter7_2/userMenu.jsp");
        } catch (UserException e) {
            request.setAttribute("msg", e.getMessage());
            request.setAttribute("form", user);
            request.getRequestDispatcher("Chapter7_2/Back_Book.jsp").forward(request, response);
        }
    }
